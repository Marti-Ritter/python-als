...
@function
def is_detectable(a=None,c=None,tol=None,*args,**kwargs):
    varargin = is_detectable.varargin
    nargin = is_detectable.nargin
    ...
    if (nargin < 1):
        ...
        if (isstruct(a)):
            ...
            if (nargin == 2):
                tol=copy(c)
# inst\is_detectable.m:48
            else:
                if (nargin > 2):
                    usage('[retval,U] = is_detectable(sys {, tol})')
            a,b,c=sys2ss(a,nargout=3)
# inst\is_detectable.m:52
        else:
            if (nargin > 3):
                usage('[retval,U] = is_detectable(a , c {, tol})')
    
    if (exist('tol')):
        retval,U=is_stabilizable(a.T,c.T,tol,nargout=2)
# inst\is_detectable.m:57
    else:
        retval,U=is_stabilizable(a.T,c.T,nargout=2)
# inst\is_detectable.m:59
