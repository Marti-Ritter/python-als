...
- all Fortran-style order="F" arguments replaced with order="C" (numpy default), function calls adjusted accordingly
"""
...
from .numba_als_utils import (symtran, ECM_iter, sdp_QR_mrQ, celldiag, _estimate_states,
                              _calculate_autocorrelations, _form_autocovariance_matrices)
...
def als_diag(data, N, model, estimator):
    ...
    for i in range(N):
        ...
        Eyy.append(temp)
    Eyy = np.vstack(Eyy)
    Eyy = Eyy.T.ravel(order="C")  # Octave uses Fortran (column-major) order, numpy by default uses C (row-major)
...
def als_sdp_mrQ(data, N, model, estimator, rho_values=None, tracestates=None, rform=None,
                weight=None, plot=True, use_sdp=True, show_warnings=True):
    """
    ...
    datapts = data.datapts
    Aa = np.ascontiguousarray(np.atleast_2d(model.A))
    Ca = np.ascontiguousarray(np.atleast_2d(model.C))

    if hasattr(model, 'G'):
        Ga = np.ascontiguousarray(np.atleast_2d(model.G))
    else:
        Ga = np.eye(*Aa.shape)
    ...
    if hasattr(model, 'B'):
        Ba = np.ascontiguousarray(np.atleast_2d(model.B))
    else:
        Ba = np.zeros((na, na))
        data.uk = np.zeros((na, datapts))
    ...
    if hasattr(estimator, 'Q') and hasattr(estimator, 'R'):
        Q0 = np.ascontiguousarray(celldiag([estimator.Q, estimator.R]))
    else:
        Q0 = np.eye(ga + pa)

    # Calculate estimator gain if not supplied:
    if hasattr(estimator, 'L'):
        L = np.ascontiguousarray(np.atleast_2d(estimator.L))
    else:
        L, _P, _E = dlqe(Aa, Ga, Ca, estimator.Q, estimator.R, method="scipy")
    ...
    # Estimator Simulation
    y = np.ascontiguousarray(data.yk.T).T
    u = np.ascontiguousarray(data.uk.T).T

    # Estimate states
    if not hasattr(data, 'xhatk'):
        xhat0 = np.ascontiguousarray(model.xhat0)
        xhat_ = _estimate_states(na, datapts, xhat0, Aa, Ba, Ca, L, y, u)
        """
        xhat = np.zeros((na, datapts))
        xhat_ = np.zeros((n, datapts + 1))
        xhat_[:n, 0] = model.xhat0
    ...
    # Calculation of Autocorrelations for one column ALS
    Eyy, Wm = _calculate_autocorrelations(datapts, start, inntrun, N, pa, dataweight=dataweight)
    ...
    M = np.kron(Ca, np.eye(na)) @ M1 @ symtran(ga)
    Mrank = np.linalg.matrix_rank(M, tol=0.0001)
    nr, nc = M.shape

    if nc > Mrank and show_warnings:
    ...
    if not sdp:
        ...
        if lam_vec is not None and show_warnings:
        ...
        Qest_cell.append((symtran(ga) @ X[:ga * (ga + 1) // 2]).reshape((ga, ga), order="C"))
        if Rsym == 1:
            Rest_cell.append((symtran(pa) @ X[ga * (ga + 1) // 2:]).reshape((pa, pa), ordeR="C"))
        else:
        ...
        # ensure that errors are raised
        # this should help ensure that we do not end up working on invalid (e.g. inf, nan) values
        # especially important since the current defaults appear to be to just warn for divide, overflow, and invalid
        # it also appears that underflows are still not raised, but I am not sure if that is a problem
        ...
        for i in range(len(lam_iterator)):
            ...
            QR1, phi, phi_tot, n_iters, nsq, iter_maxed = res[:6]
            Qest = QR1[:ga, :ga]
            Rest = QR1[ga:, ga:]
    ...
    bhat_mtr, bhat_mtr_thry, cov_bound = _form_autocovariance_matrices(Rsym, LHSsingc, ga, Qest_cell[0], Rest_cell[0],
                                                                       Eyy, N, pa, datapts, start)
